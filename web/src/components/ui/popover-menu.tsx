...
export const PopoverMenuTrigger = ({ className, ...props }: PopoverMenuTriggerProps) => {
  return (
    <PopoverMenuPrimitive.Trigger
      className={twMerge(
        'focus-visible:ring-primary hover:ring-primary bg-foreground hover:bg-primary active:bg-primary/90 group flex items-center justify-center rounded-lg p-2 outline-none transition-all focus-visible:ring-1 active:transition-none',
        className,
      )}
      {...props}
    />
  ...
  return (
    <PopoverMenuPrimitive.Portal>
      <PopoverMenuPrimitive.Content
        className={twMerge(
          'bg-background min-w-28 rounded-xl border shadow data-[state=closed]:animate-[content-hide_200ms] data-[state=open]:animate-[content-show_200ms]',
          className,
        )}
        {...props}
      />
